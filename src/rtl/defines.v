...
//Decode info bus macros
`define DECINFO_GRP_WIDTH 2 //4 group in total(ALU, BJP, MULDIV, AGU)
`define DECINFO_GRP_ALU `DECINFO_GRP_WIDTH'b0
`define DECINFO_GRP_BJP `DECINFO_GRP_WIDTH'b1
`define DECINFO_GRP_MULDIV `DECINFO_GRP_WIDTH'b2
`define DECINFO_GRP_AGU `DECINFO_GRP_WIDTH'b3

`define DECINFO_GRP_LSB 0 //low bit of GRP segment
`define DECINFO_GRP_MSB `DECINFO_GRP_LSB+`DECINFO_GRP_WIDTH - 1//high bit og GRP segment
`define DECINFO_GRP `DECINFO_GRP_MSB:`DECINFO_GRP_LSB

`define DECINFO_BJP_JUMP `DECINFO_GRP_MSB+1
`define DECINFO_BJP_BPRDT `DECINFO_BJP_JUMP+1
`define DECINFO_BJP_BEQ `DECINFO_BJP_BPRDT+1
`define DECINFO_BJP_BNE `DECINFO_BJP_BPRDT+1
`define DECINFO_BJP_BLT `DECINFO_BJP_BNE+1
`define DECINFO_BJP_BGE `DECINFO_BJP_BLT+1 
`define DECINFO_BJP_BLTU `DECINFO_BJP_BGE+1
`define DECINFO_BJP_BGEU `DECINFO_BJP_BLTU+1 
`define DECINFO_BJP_BXX `DECINFO_BJP_BGEU+1
`define DECINFO_BJP_WIDTH `DECINFO_BJP_BXX+1

`define DECINFO_ALU_ADD `DECINFO_GRP_MSB+1
`define DECINFO_ALU_SUB `DECINFO_ALU_ADD+1
`define DECINFO_ALU_SLT `DECINFO_ALU_SUB+1 
`define DECINFO_ALU_SLTU `DECINFO_ALU_SLT+1 
`define DECINFO_ALU_XOR `DECINFO_ALU_SLTU+1 
`define DECINFO_ALU_OR `DECINFO_ALU_XOR+1 
`define DECINFO_ALU_AND `DECINFO_ALU_OR+1 
`define DECINFO_ALU_SLL`DECINFO_ALU_AND+1 
`define DECINFO_ALU_SRL `DECINFO_ALU_SLL+1 
`define DECINFO_ALU_SRA `DECINFO_ALU_SRL+1 
`define DECINFO_ALU_LUI `DECINFO_ALU_SRA+1
...
`define DECINFO_ALU_WIDTH `DECINFO_ALU_NOP+1

`define DECINFO_MULDIV_MUL `DECINFO_GRP_MSB+1
...
`define DECINFO_AGU_LOAD `DECINFO_GRP_MSB+1
...
`define DECINFO_AGU_OP2IMM `DECINFO_AGU_USIGN+1
